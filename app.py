    ...
    return render_template_string('''
        ...
        <body style="font-family:sans-serif; padding:40px; background:#0b0f19; color:white;">
            <div style="background:#1b2234; padding:30px; border-radius:8px; max-width:800px; margin:0 auto; border:1px solid #2b354f;">
                ...
                <div id="result" style="margin-top:20px; word-break:break-all; background:#000; padding:15px; display:none; border:1px solid #FFD700; color:#FFD700; font-family:monospace;"></div>
            </div>
            <script>
                function generate() {
                    let input = document.getElementById('targetUrl').value.trim();
                    if (!input) return;
                    const currentHost = "{{ host_url }}";
                    const finalUrl = `${currentHost}/proxy?url=${encodeURIComponent(input)}`;
...
def proxy():
    if request.method == 'OPTIONS':
        return Response("", headers={'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET, OPTIONS', 'Access-Control-Allow-Headers': '*'})
