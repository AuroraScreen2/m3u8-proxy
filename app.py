...
@app.route('/')
def index():
    # Automatically get the current domain (e.g., https://aurora-proxy.onrender.com)
    ...
    return render_template_string('''
        ...
        <body style="font-family:sans-serif; padding:40px; background:#111; color:white;">
            <div style="background:#222; padding:30px; border-radius:8px; max-width:800px; margin:0 auto; border:1px solid #444;">
                <h2 style="color:#00ff99;">Aurora Proxy (Render Edition)</h2>
...
def proxy():
    ...
    try:
        # TIMEOUT increased to 20s for Render
        ...
        if target_url.endswith('.m3u8') or 'mpegurl' in content_type:
            ...
            new_content = re.sub(r'URI=(["\']?)([^",\s]+)(["\']?)', 
                                 lambda m: f'URI={m.group(1)}{own_url}/proxy?url={quote(urljoin(base_url, m.group(2)))}&referer={quote(current_referer)}{m.group(3)}', 
                                 new_content)
